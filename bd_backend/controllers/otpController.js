...
exports.sendOtp = async (req, res) => {
    ...
    try {
        ...
        //if (otp) return res.status(200).send('OTP sent successfully');
        
        const response = await axios.get(`https://pgapi.vispl.in/fe/api/v1/multiSend?username=aditrpg1.trans&password=Ad1tya@1234&unicode=false&from=ADIUNV&to=${phoneNumber}&text=Your+OTP+for+blood+donation+registration+is+${otp}.+Please+complete+verification+within+5+minutes.+Do+not+share+this+with+anyone.@Aditya+University`);
